window.define||(window.define=function(a,b,c){moqui.isString(a)||(c=b,b=a);moqui.isArray(b)||(c=b);return moqui.isFunction(c)?c():c});moqui.localeMap={zh:"zh-cn"};moqui.objToSearch=function(a){var b="";$.each(a,function(a,d){b=b+(0<b.length?"\x26":"")+a+"\x3d"+d});return b};moqui.searchToObj=function(a){if(!a||0===a.length)return{};var b={};a=a.split("\x26");for(var c=0;c<a.length;c++){var d=a[c].split("\x3d");if(1<d.length){var e=d[0],d=d[1],f=b[e];f?moqui.isArray(f)?f.push(d):b[e]=[f,d]:b[e]=d}}return b};
moqui.decodeHtml=function(a){var b=document.createElement("textarea");b.innerHTML=a;return b.value};Vue.filter("decodeHtml",moqui.decodeHtml);
moqui.format=function(a,b,c){b&&b.length&&(b=b.replace(/a/,"A").replace(/d/,"D").replace(/y/,"Y"));if(c&&c.length){c=c.toLowerCase();if("date"===c)return b&&0!==b.length||(b="YYYY-MM-DD"),moment(a).format(b);if("time"===c)return b&&0!==b.length||(b="HH:mm:ss"),moment(a).format(b);if("timestamp"===c)return b&&0!==b.length||(b="YYYY-MM-DD HH:mm"),moment(a).format(b);if("bigdecimal"===c||"long"===c||"integer"===c||"double"===c||"float"===c)return a;console.warn("format type unknown: "+c)}return!moqui.isNumber(a)&&
(c=moment(a),c.isValid())?(b&&0!==b.length||(b="YYYY-MM-DD HH:mm"),c.format(b)):a};Vue.filter("format",moqui.format);moqui.loadScript=function(a){var b=!1;$("head script").each(function(c,e){-1!==e.src.indexOf(a)&&(b=!0)});if(!b){var c=document.createElement("script");c.src=a;c.async=!1;document.head.appendChild(c)}};
moqui.loadStylesheet=function(a,b,c){b||(b="stylesheet");c||(c="text/css");var d=!1;$("head link").each(function(b,c){-1!==c.href.indexOf(a)&&(d=!0)});if(!d){var e=document.createElement("link");e.href=a;e.rel=b;e.type=c;document.head.appendChild(e)}};moqui.retryInlineScript=function(a,b){try{eval(a)}catch(d){a=a.trim();var c=5>=b?b*b*100:"N/A";console.warn("inline script error "+b+" retry "+c+" script: "+a.slice(0,80)+"...");console.warn(d);5>=b&&setTimeout(moqui.retryInlineScript,c,a,b+1)}};
moqui.notifyOpts={delay:1500,timer:250,offset:{x:20,y:60},placement:{from:"top",align:"right"},z_index:1100,type:"success",animate:{enter:"animated fadeInDown",exit:""}};moqui.notifyOptsInfo={delay:3E3,timer:250,offset:{x:20,y:60},placement:{from:"top",align:"right"},z_index:1100,type:"info",animate:{enter:"animated fadeInDown",exit:""}};
moqui.notifyOptsError={delay:5E3,timer:250,offset:{x:20,y:60},placement:{from:"top",align:"right"},z_index:1100,type:"danger",animate:{enter:"animated fadeInDown",exit:""}};
moqui.notifyMessages=function(a,b,c){var d=!1;if(a)if(moqui.isArray(a))for(var e=0;e<a.length;e++){d=a[e];if(moqui.isPlainObject(d)){var f=d.type;f&&f.length||(f="info");$.notify({message:d.message},$.extend({},moqui.notifyOptsInfo,{type:f}));moqui.webrootVue.addNotify(d.message,f)}else $.notify({message:d},moqui.notifyOptsInfo),moqui.webrootVue.addNotify(d,"info");d=!0}else $.notify({message:a},moqui.notifyOptsInfo),moqui.webrootVue.addNotify(a,"info"),d=!0;if(b)if(moqui.isArray(b))for(a=0;a<b.length;a++)$.notify({message:b[a]},
moqui.notifyOptsError),moqui.webrootVue.addNotify(b[a],"danger"),d=!0;else $.notify({message:b},moqui.notifyOptsError),moqui.webrootVue.addNotify(b,"danger"),d=!0;if(c)if(moqui.isArray(c))for(b=0;b<c.length;b++)moqui.notifyValidationError(c[b]),d=!0;else moqui.notifyValidationError(c),d=!0;return d};
moqui.notifyValidationError=function(a){var b=a;moqui.isPlainObject(a)&&(b=a.message,a.fieldPretty&&a.fieldPretty.length&&(b=b+" (for field "+a.fieldPretty+")"));$.notify({message:b},moqui.notifyOptsError);moqui.webrootVue.addNotify(b,"danger")};
moqui.handleAjaxError=function(a,b,c){var d=a.responseText,e;try{e=JSON.parse(d)}catch(g){}console.warn("ajax "+b+" ("+a.status+"), message "+c);var f=!1;e&&moqui.isPlainObject(e)?f=moqui.notifyMessages(e.messageInfos,e.errors,e.validationErrors):d&&moqui.isString(d)&&d.length&&(f=moqui.notifyMessages(d));401===a.status?moqui.webrootVue?window.location.href=moqui.webrootVue.currentLinkUrl:window.location.reload(!0):0===a.status?0>c.indexOf("abort")&&($.notify({message:"Could not connect to server"},
moqui.notifyOptsError),moqui.webrootVue.addNotify("Could not connect to server","danger")):f||(a="Error: "+c+" ("+b+")",$.notify({message:a},moqui.notifyOptsError),moqui.webrootVue.addNotify(a,"danger"))};moqui.componentCache=new moqui.LruMap(50);moqui.handleLoadError=function(a,b,c){moqui.webrootVue.loading=0;moqui.handleAjaxError(a,b,c)};
moqui.loadComponent=function(a,b,c){var d,e,f,g;if("string"===typeof a){var h=a.indexOf("?");0<h?(d=a.slice(0,h),f=a.slice(h+1)):d=a}else d=a.path,e=a.extraPath,f=a.search,g=a.bodyParameters;if(h=moqui.componentCache.get(d))console.info("found cached component for path "+d),b(h);else{var k=d,m=".js"===d.slice(-3);!m&&a.renderModes&&0<=a.renderModes.indexOf("js")&&(k+=".js",m=!0);m||(k+=".vuet");e&&0<e.length&&(k+="/"+e);f&&0<f.length&&(k+="?"+f);console.info("loadComponent "+k+(c?" id "+c:""));a=
{type:"GET",url:k,error:moqui.handleLoadError,success:function(a,e,f){a||b(moqui.NotFound);var g=0<=f.getResponseHeader("Cache-Control").indexOf("max-age");if(moqui.isString(a)&&0<a.length)if(m||"define("===a.slice(0,7)){console.info("loaded JS from "+k+(c?" id "+c:""));var h=eval(a);h.template?(g&&moqui.componentCache.put(d,h),b(h)):(a=(".js"===d.slice(-3)?d.slice(0,-3):d)+".vuet",$.ajax({type:"GET",url:a,error:moqui.handleLoadError,success:function(a){h.template=a;g&&moqui.componentCache.put(d,
h);b(h)}}))}else a=a.replace(/<script/g,"\x3cm-script").replace(/<\/script>/g,"\x3c/m-script\x3e").replace(/<link/g,"\x3cm-stylesheet"),console.info("loaded HTML template from "+k+(c?" id "+c:"")),a={template:"\x3cdiv"+(c&&0<c.length?' id\x3d"'+c+'"':"")+"\x3e"+a+"\x3c/div\x3e"},g&&moqui.componentCache.put(d,a),b(a);else moqui.isPlainObject(a)?a.screenUrl&&0<a.screenUrl.length?this.$root.setUrl(a.screenUrl):a.redirectUrl&&0<a.redirectUrl.length&&window.location.replace(a.redirectUrl):b(moqui.NotFound)}};
g&&!$.isEmptyObject(g)&&(a.type="POST",a.data=g);$.ajax(a)}};moqui.NotFound=Vue.extend({template:'\x3cdiv id\x3d"current-page-root"\x3e\x3ch4\x3eScreen not found at {{this.$root.currentPath}}\x3c/h4\x3e\x3c/div\x3e'});moqui.EmptyComponent=Vue.extend({template:'\x3cdiv id\x3d"current-page-root"\x3e\x3cdiv class\x3d"spinner"\x3e\x3cdiv\x3eLoading…\x3c/div\x3e\x3c/div\x3e\x3c/div\x3e'});
...
methods:{hide:function(){$(this.$el).modal("hide")}},mounted:function(){var a=$(this.$el),b=this;a.on("hidden.bs.modal",function(){b.isHidden=!0});a.on("shown.bs.modal",function(){b.isHidden=!1;a.find(":not(.noResetSelect2)\x3eselect:not(.noResetSelect2)").select2({});var c=a.find(".default-focus");c.length?c.focus():a.find("form :input:visible:first").focus()});this.openDialog&&a.modal("show")}});
...
Vue.component("dynamic-dialog",{props:{id:{type:String,required:!0},url:{type:String,required:!0},title:String,width:{type:String,"default":"760"},openDialog:{type:Boolean,"default":!1}},data:function(){var a=$(window).width();return{curComponent:moqui.EmptyComponent,curUrl:"",isHidden:!0,dialogStyle:{width:(this.width<a?this.width:a)+"px"}}},template:'\x3cdiv :id\x3d"id" class\x3d"modal dynamic-dialog" aria-hidden\x3d"true" style\x3d"display: none;" tabindex\x3d"-1"\x3e\x3cdiv class\x3d"modal-dialog" :style\x3d"dialogStyle"\x3e\x3cdiv class\x3d"modal-content"\x3e\x3cdiv class\x3d"modal-header"\x3e\x3cbutton type\x3d"button" class\x3d"close" data-dismiss\x3d"modal" aria-hidden\x3d"true"\x3e\x26times;\x3c/button\x3e\x3ch4 class\x3d"modal-title"\x3e{{title}}\x3c/h4\x3e\x3c/div\x3e\x3cdiv class\x3d"modal-body"\x3e\x3ccomponent :is\x3d"curComponent"\x3e\x3c/component\x3e\x3c/div\x3e\x3c/div\x3e\x3c/div\x3e\x3c/div\x3e',methods:{reload:function(){if(!this.isHidden){var a=
$(this.$el);a.modal("hide");a.modal("show")}},load:function(a){this.curUrl=a},hide:function(){$(this.$el).modal("hide")}},watch:{curUrl:function(a){if(a&&0!==a.length){var b=this;moqui.loadComponent(a,function(a){a.mounted=function(){var a=$(b.$el);a.find(":not(.noResetSelect2)\x3eselect:not(.noResetSelect2)").select2({});var c=a.find(".default-focus");c.length?c.focus():a.find("form :input:visible:first").focus()};b.curComponent=a},this.id)}else this.curComponent=moqui.EmptyComponent}},mounted:function(){this.$root.addContainer(this.id,
this);var a=$(this.$el),b=this;a.on("show.bs.modal",function(){b.curUrl=b.url});a.on("hidden.bs.modal",function(){b.isHidden=!0;b.curUrl=""});a.on("shown.bs.modal",function(){b.isHidden=!1});this.openDialog&&a.modal("show")}});
...
Vue.component("tree-item",{template:'\x3cli :id\x3d"model.id"\x3e\x3ci v-if\x3d"isFolder" @click\x3d"toggle" class\x3d"glyphicon" :class\x3d"{\'glyphicon-chevron-right\':!open, \'glyphicon-chevron-down\':open}"\x3e\x3c/i\x3e\x3ci v-else class\x3d"glyphicon glyphicon-unchecked"\x3e\x3c/i\x3e \x3cspan @click\x3d"setSelected"\x3e\x3cm-link v-if\x3d"model.a_attr" :href\x3d"model.a_attr.urlText" :load-id\x3d"model.a_attr.loadId" :class\x3d"{\'text-success\':selected}"\x3e{{model.text}}\x3c/m-link\x3e\x3cspan v-if\x3d"!model.a_attr" :class\x3d"{\'text-success\':selected}"\x3e{{model.text}}\x3c/span\x3e\x3c/span\x3e\x3cul v-show\x3d"open" v-if\x3d"hasChildren"\x3e\x3ctree-item v-for\x3d"model in model.children" :key\x3d"model.id" :model\x3d"model" :top\x3d"top"/\x3e\x3c/ul\x3e\x3c/li\x3e',props:{model:Object,
...
Vue.component("m-form",{props:{action:{type:String,required:!0},method:{type:String,"default":"POST"},submitMessage:String,submitReloadId:String,submitHideId:String,focusField:String,noValidate:Boolean},data:function(){return{fields:{},fieldsChanged:{}}},template:'\x3cform @submit.prevent\x3d"submitForm"\x3e\x3cslot\x3e\x3c/slot\x3e\x3c/form\x3e',methods:{submitForm:function(){var a=$(this.$el);if(this.noValidate||a.valid()){var b=null,c=null,d=$(document.activeElement);d.length&&a.has(d)&&d.is('button[type\x3d"submit"], input[type\x3d"submit"], input[type\x3d"image"]')&&
(d.is("[name]")&&(b=d.attr("name"),c=d.val()),d.prop("disabled",!0),setTimeout(function(){d.prop("disabled",!1)},3E3));var e=new FormData(this.$el);e.append("moquiSessionToken",this.$root.moquiSessionToken);$.each(this.fields,function(a,b){e.append(a,b)});b&&e.append(b,c);this.$root.loading++;$.ajax({type:this.method,url:this.$root.appRootPath+this.action,data:e,contentType:!1,processData:!1,headers:{Accept:"application/json"},error:moqui.handleLoadError,success:this.handleResponse})}else if(!this.noValidate){var f=
a.find("div.has-error input, div.has-error select, div.has-error textarea").first();f.length&&(a=f.parents("div.panel-collapse.collapse").last(),a.length?a.hasClass("in")?f.focus():(a=a.prevAll("div.panel-heading").last(),a.length?(b=a.find('a[data-toggle\x3d"collapse"]').first(),b.length?b.click():a.click(),setTimeout(function(){f.focus()},250)):f.focus()):f.focus())}},handleResponse:function(a){this.$root.loading--;var b=!1;a&&moqui.isPlainObject(a)?(b=moqui.notifyMessages(a.messageInfos,a.errors),
a.screenUrl&&0<a.screenUrl.length?this.$root.setUrl(a.screenUrl):a.redirectUrl&&0<a.redirectUrl.length&&(window.location.href=a.redirectUrl)):console.warn("m-form no response or non-JSON response: "+JSON.stringify(a));(a=this.submitHideId)&&0<a.length&&$("#"+a).modal("hide");(a=this.submitReloadId)&&0<a.length&&this.$root.reloadContainer(a);(a=this.submitMessage)&&a.length?(b=eval('"'+a+'"'),$.notify({message:b},moqui.notifyOpts),moqui.webrootVue.addNotify(b,"success")):b||$.notify({message:"Form data saved"},
moqui.notifyOpts)},fieldChange:function(a){a=a.delegateTarget;var b=$(a);b.hasClass("input-group")&&b.children("input").length&&(b=b.children("input").first(),a=b.get(0));var c=!1;if("INPUT"===a.nodeName||"TEXTAREA"===a.nodeName)c="radio"===b.attr("type")||"checkbox"===b.attr("type")?a.checked!==a.defaultChecked:a.value!==a.defaultValue;else if("SELECT"===a.nodeName)if(a.multiple)for(var d=a.options.length,e=0;e<d;e++){var f=a.options[e];if(f.selected!==f.defaultSelected){c=!0;break}}else c=!a.options[a.selectedIndex].defaultSelected;
c?(this.fieldsChanged[b.attr("name")]=!0,b.parents(".form-group").children("label").addClass("is-changed"),b.parents(".form-group").find(".select2-selection").addClass("is-changed"),b.addClass("is-changed")):(this.fieldsChanged[b.attr("name")]=!1,b.parents(".form-group").children("label").removeClass("is-changed"),b.parents(".form-group").find(".select2-selection").removeClass("is-changed"),b.removeClass("is-changed"))}},mounted:function(){var a=$(this.$el);this.noValidate||a.validate({errorClass:"help-block",
errorElement:"span",highlight:function(a,c,d){$(a).parents(".form-group").removeClass("has-success").addClass("has-error")},unhighlight:function(a,c,d){$(a).parents(".form-group").removeClass("has-error").addClass("has-success")}});a.find('[data-toggle\x3d"tooltip"]').tooltip({placement:"auto top"});this.focusField&&0<this.focusField.length&&a.find('[name^\x3d"'+this.focusField+'"]').addClass("default-focus").focus();a.find(":input").on("change",this.fieldChange);a.find("div.input-group.date").on("change",
this.fieldChange)}});
Vue.component("form-link",{props:{action:{type:String,required:!0},focusField:String,noValidate:Boolean,bodyParameterNames:Array},data:function(){return{fields:{}}},template:'\x3cform @submit.prevent\x3d"submitForm"\x3e\x3cslot :clearForm\x3d"clearForm"\x3e\x3c/slot\x3e\x3c/form\x3e',methods:{submitForm:function(){var a=$(this.$el);if(this.noValidate||a.valid()){var b=null,c=null,d=$(document.activeElement);d.length&&a.has(d)&&d.is('button[type\x3d"submit"], input[type\x3d"submit"], input[type\x3d"image"]')&&(d.is("[name]")&&
(b=d.attr("name"),c=d.val()),d.prop("disabled",!0),setTimeout(function(){d.prop("disabled",!1)},3E3));var e=a.serializeArray();$.each(this.fields,function(a,b){e.push({name:a,value:b})});for(var f=[],g=[],a="",h=null,k=0;k<e.length;k++){var m=e[k],l=m.name,n=m.value;0===n.trim().length||"moquiSessionToken"===l||"moquiFormName"===l||0<l.indexOf("[]")||(0<l.indexOf("_op")||0<l.indexOf("_not")||0<l.indexOf("_ic")?f.push(m):(g.push(l),this.bodyParameterNames&&0<=this.bodyParameterNames.indexOf(l)?(h||
(h={}),h[l]=n):(0<a.length&&(a+="\x26"),a+=encodeURIComponent(l)+"\x3d"+encodeURIComponent(n))))}for(k=0;k<f.length;k++)m=f[k],l=m.name.substring(0,m.name.indexOf("_")),0<=g.indexOf(l)&&(0<a.length&&(a+="\x26"),a+=encodeURIComponent(m.name)+"\x3d"+encodeURIComponent(m.value));b&&c&&c.trim().length&&(0<a.length&&(a+="\x26"),a+=encodeURIComponent(b)+"\x3d"+encodeURIComponent(c));b=this.action;b=0<b.indexOf("?")?b+"\x26"+a:b+"?"+a;this.$root.setUrl(b,h)}},clearForm:function(){var a=$(this.$el);a.find(":radio, :checkbox").removeAttr("checked");
...
Vue.component("form-paginate",{props:{paginate:Object,formList:Object},template:'\x3cul v-if\x3d"paginate" class\x3d"pagination"\x3e\x3ctemplate v-if\x3d"paginate.pageIndex \x3e 0"\x3e\x3cli\x3e\x3ca href\x3d"#" @click.prevent\x3d"setIndex(0)"\x3e\x3ci class\x3d"glyphicon glyphicon-fast-backward"\x3e\x3c/i\x3e\x3c/a\x3e\x3c/li\x3e\x3cli\x3e\x3ca href\x3d"#" @click.prevent\x3d"setIndex(paginate.pageIndex-1)"\x3e\x3ci class\x3d"glyphicon glyphicon-backward"\x3e\x3c/i\x3e\x3c/a\x3e\x3c/li\x3e\x3c/template\x3e\x3ctemplate v-else\x3e\x3cli\x3e\x3cspan\x3e\x3ci class\x3d"glyphicon glyphicon-fast-backward"\x3e\x3c/i\x3e\x3c/span\x3e\x3c/li\x3e\x3cli\x3e\x3cspan\x3e\x3ci class\x3d"glyphicon glyphicon-backward"\x3e\x3c/i\x3e\x3c/span\x3e\x3c/li\x3e\x3c/template\x3e\x3cli v-for\x3d"prevIndex in prevArray"\x3e\x3ca href\x3d"#" @click.prevent\x3d"setIndex(prevIndex)"\x3e{{prevIndex+1}}\x3c/a\x3e\x3c/li\x3e\x3cli\x3e\x3cspan\x3ePage {{paginate.pageIndex+1}} of {{paginate.pageMaxIndex+1}} ({{paginate.pageRangeLow}} - {{paginate.pageRangeHigh}} of {{paginate.count}})\x3c/span\x3e\x3c/li\x3e\x3cli v-for\x3d"nextIndex in nextArray"\x3e\x3ca href\x3d"#" @click.prevent\x3d"setIndex(nextIndex)"\x3e{{nextIndex+1}}\x3c/a\x3e\x3c/li\x3e\x3ctemplate v-if\x3d"paginate.pageIndex \x3c paginate.pageMaxIndex"\x3e\x3cli\x3e\x3ca href\x3d"#" @click.prevent\x3d"setIndex(paginate.pageIndex+1)"\x3e\x3ci class\x3d"glyphicon glyphicon-forward"\x3e\x3c/i\x3e\x3c/a\x3e\x3c/li\x3e\x3cli\x3e\x3ca href\x3d"#" @click.prevent\x3d"setIndex(paginate.pageMaxIndex)"\x3e\x3ci class\x3d"glyphicon glyphicon-fast-forward"\x3e\x3c/i\x3e\x3c/a\x3e\x3c/li\x3e\x3c/template\x3e\x3ctemplate v-else\x3e\x3cli\x3e\x3cspan\x3e\x3ci class\x3d"glyphicon glyphicon-forward"\x3e\x3c/i\x3e\x3c/span\x3e\x3c/li\x3e\x3cli\x3e\x3cspan\x3e\x3ci class\x3d"glyphicon glyphicon-fast-forward"\x3e\x3c/i\x3e\x3c/span\x3e\x3c/li\x3e\x3c/template\x3e\x3c/ul\x3e',computed:{prevArray:function(){var a=
...
this.fetchRows()}});
Vue.component("date-time",{props:{id:String,name:{type:String,required:!0},value:String,type:{type:String,"default":"date-time"},size:String,format:String,tooltip:String,form:String,required:String,autoYear:String},template:'\x3cdiv v-if\x3d"type\x3d\x3d\'time\'" class\x3d"input-group time" :id\x3d"id"\x3e\x3cinput type\x3d"text" class\x3d"form-control" :pattern\x3d"timePattern" :id\x3d"id?(id+\'_itime\'):\'\'" :name\x3d"name" :value\x3d"value" :size\x3d"sizeVal" :data-toggle\x3d"{tooltip:(tooltip\x26\x26tooltip.length\x3e0)}" :title\x3d"tooltip" :form\x3d"form"\x3e\x3cspan class\x3d"input-group-addon"\x3e\x3cspan class\x3d"glyphicon glyphicon-time"\x3e\x3c/span\x3e\x3c/span\x3e\x3c/div\x3e\x3cdiv v-else class\x3d"input-group date" :id\x3d"id"\x3e\x3cinput ref\x3d"dateInput" @focus\x3d"focusDate" @blur\x3d"blurDate" type\x3d"text" class\x3d"form-control" :id\x3d"id?(id+\'_idate\'):\'\'" :name\x3d"name" :value\x3d"value" :size\x3d"sizeVal" :data-toggle\x3d"{tooltip:(tooltip\x26\x26tooltip.length\x3e0)}" :title\x3d"tooltip" :form\x3d"form" :required\x3d"required \x3d\x3d \'required\' ? true : false"\x3e\x3cspan class\x3d"input-group-addon"\x3e\x3cspan class\x3d"glyphicon glyphicon-calendar"\x3e\x3c/span\x3e\x3c/span\x3e\x3c/div\x3e',methods:{focusDate:function(){if("time"!==
...
Vue.component("drop-down",{props:{options:Array,value:[Array,String],combo:Boolean,allowEmpty:Boolean,multiple:String,optionsUrl:String,serverSearch:{type:Boolean,"default":!1},serverDelay:{type:Number,"default":300},serverMinLength:{type:Number,"default":1},optionsParameters:Object,labelField:String,valueField:String,dependsOn:Object,dependsOptional:Boolean,optionsLoadInit:Boolean,form:String},data:function(){return{curData:null,s2Opts:null,lastVal:null}},template:'\x3cselect :form\x3d"form"\x3e\x3cslot\x3e\x3c/slot\x3e\x3c/select\x3e',
...
this.serverMinLength,c.minimumResultsForSearch=0,c.width="100%",a.css("min-width","200px"),a.addClass("noResetSelect2"));this.s2Opts=c;a.select2(c).on("change",function(){b.$emit("input",this.value)});var d=this.value;d&&d.length&&(this.curVal=d);if(this.optionsUrl&&0<this.optionsUrl.length){var a=this.dependsOn,e;for(e in a)if(a.hasOwnProperty(e))$("#"+a[e]).on("change",function(){b.populateFromUrl({term:d})});this.optionsLoadInit&&(this.serverSearch?d&&d.length&&moqui.isString(d)&&this.populateFromUrl({term:d}):
this.populateFromUrl())}},computed:{curVal:{get:function(){return $(this.$el).select2().val()},set:function(a){$(this.$el).val(a).trigger("change")}}},watch:{value:function(a){this.curVal=a},options:function(a){this.curData=a},curData:function(a){var b=$(this.$el),c=this,d=b.next().hasClass("select2-container--focus"),e=b.select2().val();e&&1<e.length&&(this.lastVal=e);b.select2("destroy");b.empty();this.s2Opts.data=a;b.select2(this.s2Opts).on("change",function(){c.$emit("input",this.value)});d&&
b.focus();setTimeout(function(){var d=c.lastVal;if(!d||2>d.length)d=c.value;if(d){var e=!1,h=moqui.isArray(d);$.each(a,function(a,b){if(h?$.inArray(b.id,d):b.id===d)e=!0});e&&b.val(d)}b.trigger("change")},50)}},destroyed:function(){$(this.$el).off().select2("destroy")}});
...
"/"+d.slice(0,c+1).join("/");if(!f&&moqui.componentCache.containsKey(e))return!1;f={path:e};c===d.length-1&&(d=b.extraPathList)&&0<d.length&&(f.extraPath=d.join("/"));(d=b.currentSearch)&&0<d.length&&(f.search=d);f.bodyParameters=b.bodyParameters;(d=b.navMenuList[c+b.basePathSize])&&d.renderModes&&(f.renderModes=d.renderModes);console.info("subscreens-active loadActive pathIndex "+c+" pathName "+a.pathName+" urlInfo "+JSON.stringify(f));b.loading++;moqui.loadComponent(f,function(c){a.activeComponent=
c;b.loading--});return!0}},mounted:function(){this.$root.addSubscreen(this)}});
moqui.webrootVue=new Vue({el:"#apps-root",data:{basePath:"",linkBasePath:"",currentPathList:[],extraPathList:[],activeSubscreens:[],currentParameters:{},bodyParameters:null,navMenuList:[],navHistoryList:[],navPlugins:[],notifyHistoryList:[],lastNavTime:Date.now(),loading:0,activeContainers:{},moquiSessionToken:"",appHost:"",appRootPath:"",userId:"",locale:"en",notificationClient:null},methods:{setUrl:function(a,b){this.bodyParameters=b;$(".modal.in").modal("hide");a=this.getLinkPath(a);if(this.currentLinkUrl===
a&&a!==this.linkBasePath)this.reloadSubscreens();else{var c=a,d=c.indexOf("://");if(0<=d){d=c.indexOf("/",d+3);if(-1===d)return;c=c.slice(d)}c=c.split("?");this.extraPathList=[];this.currentSearch=1<c.length&&0<c[1].length?c[1]:"";this.currentPath=c[0];c=this.currentSearch;if((c=this.currentPath+(0<c.length?"?"+c:""))&&0!==c.length){console.info("current URL changing to "+c);this.lastNavTime=Date.now();this.activeContainers={};var e=this,c=this.appRootPath&&this.appRootPath.length&&0===c.indexOf(this.appRootPath)?
this.appRootPath+"/menuData"+c.slice(this.appRootPath.length):"/menuData"+c;$.ajax({type:"GET",url:c,dataType:"text",error:moqui.handleAjaxError,success:function(a){var b=null;try{b=JSON.parse(a)}catch(c){console.info("Error parson menu list JSON: "+c)}b&&moqui.isArray(b)&&(e.navMenuList=b)}});window.history.pushState(null,this.ScreenTitle,a)}}},setParameters:function(a){a&&(this.$root.currentParameters=$.extend({},this.$root.currentParameters,a));this.$root.reloadSubscreens()},addSubscreen:function(a){a.pathIndex=
...
b[d];break}c&&c.pathWithParams&&c.pathWithParams.length&&this.setUrl(c.pathWithParams)},addContainer:function(a,b){this.activeContainers[a]=b},reloadContainer:function(a){var b=this.activeContainers[a];b?b.reload():console.error("Container with ID "+a+" not found, not reloading")},loadContainer:function(a,b){var c=this.activeContainers[a];c?c.load(b):console.error("Container with ID "+a+" not found, not loading url "+b)},addNavPlugin:function(a){var b=this;moqui.loadComponent(this.appRootPath+a,function(a){b.navPlugins.push(a)})},
addNotify:function(a,b){var c=this.notifyHistoryList.slice(0),d=new Date,e=d.getHours();10>e&&(e="0"+e);d=d.getMinutes();10>d&&(d="0"+d);for(c.unshift({message:a,type:b,time:e+":"+d});25<c.length;)c.pop();this.notifyHistoryList=c},switchDarkLight:function(){var a=$("body");a.toggleClass("bg-dark");a.toggleClass("bg-light");a=a.hasClass("bg-dark")?"bg-dark":"bg-light";$.ajax({type:"POST",url:this.appRootPath+"/apps/setPreference",error:moqui.handleAjaxError,data:{moquiSessionToken:this.moquiSessionToken,
preferenceKey:"OUTER_STYLE",preferenceValue:a}})},showScreenDocDialog:function(a){$("#screen-document-dialog").modal("show");$("#screen-document-dialog-body").load(this.currentPath+"/screenDoc?docIndex\x3d"+a)},stopProp:function(a){a.stopPropagation()},getNavHref:function(a){a||(a=this.navMenuList.length-1);a=this.navMenuList[a];if(a.extraPathList&&a.extraPathList.length){var b=a.path+"/"+a.extraPathList.join("/"),c=a.pathWithParams.indexOf("?");0<c&&(b+=a.pathWithParams.slice(c));return b}return a.pathWithParams||
a.path},getLinkPath:function(a){this.appRootPath&&this.appRootPath.length&&0!==a.indexOf(this.appRootPath)&&(a=this.appRootPath+a);0===a.indexOf(this.basePath)&&(a=a.replace(this.basePath,this.linkBasePath));return a}},watch:{navMenuList:function(a){if(0<a.length){var b=a[a.length-1];a=1<a.length?a[a.length-2]:null;b.extraPathList&&(this.extraPathList=b.extraPathList);var c=this.basePathSize,d=b.path.split("/").slice(c+1);console.info("nav updated fullPath "+JSON.stringify(d)+" currentPathList "+
JSON.stringify(this.currentPathList)+" cur.path "+b.path+" basePathSize "+c);this.currentPathList=d;this.reloadSubscreens();a=(a?a.title+" - ":"")+b.title;var c=b.pathWithParams,e=c.indexOf("?");if(0<e){for(var d=["pageIndex","orderBySelect","orderByField","moquiSessionToken"],f=c.substring(e+1).split("\x26"),c=c.substring(0,e),e=0,g="",h=0;h<f.length;h++){var k=f[h],c=-1===c.indexOf("?")?c+"?":c+"\x26",c=c+k;if(!(3<e)){var m=k.indexOf("\x3d");if(0<m){var l=k.substring(0,m),k=k.substring(m+1);0<l.indexOf("_op")||
0<l.indexOf("_not")||0<l.indexOf("_ic")||0<d.indexOf(l)||l===k||(0<g.length&&(g+=", "),g+=decodeURIComponent(k),e++)}}}0<g.length&&(70<g.length&&(g=g.substring(0,70)+"..."),a=a+" ("+g+")")}d=this.navHistoryList;for(f=0;f<d.length;)d[f].pathWithParams===c?d.splice(f,1):f++;for(d.unshift({title:a,pathWithParams:c,image:b.image,imageType:b.imageType});25<d.length;)d.pop();document.title=a}},currentPathList:function(a){(a=a[a.length-1])&&$(this.$el).removeClass().addClass(a)}},computed:{currentPath:{get:function(){var a=
...
1].title:""}},created:function(){this.moquiSessionToken=$("#confMoquiSessionToken").val();this.appHost=$("#confAppHost").val();this.appRootPath=$("#confAppRootPath").val();this.basePath=$("#confBasePath").val();this.linkBasePath=$("#confLinkBasePath").val();this.userId=$("#confUserId").val();this.locale=$("#confLocale").val();moqui.localeMap[this.locale]&&(this.locale=moqui.localeMap[this.locale]);var a=this;$(".confNavPluginUrl").each(function(b,c){a.addNavPlugin($(c).val())});this.notificationClient=
new moqui.NotificationClient(("https:"===location.protocol?"wss://":"ws://")+this.appHost+this.appRootPath+"/notws")},mounted:function(){var a=$(this.$el);a.find('.navbar [data-toggle\x3d"tooltip"]').tooltip({placement:"bottom",trigger:"hover"});a.find("#history-menu-link").tooltip({placement:"bottom",trigger:"hover"});a.find("#notify-history-menu-link").tooltip({placement:"bottom",trigger:"hover"});a.find("#document-menu-link").tooltip({placement:"bottom",trigger:"hover"});this.setUrl(window.location.pathname+
window.location.search);this.notificationClient.registerListener("ALL");$("#screen-document-dialog").on("hidden.bs.modal",function(){var a=$("#screen-document-dialog-body");a.empty();a.append('\x3cdiv class\x3d"spinner"\x3e\x3cdiv\x3eLoading…\x3c/div\x3e\x3c/div\x3e')});window.Notification&&"granted"!==Notification.permission&&"denied"!==Notification.permission&&Notification.requestPermission(function(a){"granted"===a?moqui.notifyMessages("Browser notifications enabled, if you don't want them use browser notification settings to block"):
"denied"===a&&moqui.notifyMessages("Browser notifications disabled, if you want them use browser notification settings to allow")})}});window.addEventListener("popstate",function(){moqui.webrootVue.setUrl(window.location.pathname+window.location.search)});